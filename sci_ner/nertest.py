...
class BiLSTM_CRF(nn.Module):
    ...
    # 计算feature, len(sentence) * target_size
    ...
    def _get_lstm_features(self, sentence, device):
        self.hidden = self.init_hidden(device)
        embeds = self.word_embeds(sentence).view(len(sentence), 1, -1)
        lstm_out, self.hidden = self.lstm(embeds, self.hidden)
        lstm_out = lstm_out.view(len(sentence), self.hidden_dim)
        lstm_feats = self.hidden2tag(lstm_out)
        ## 返回的是将 lstm feats经过fc变成 target-size大小，最终大小变成 len(sentence)*target_size的
        return lstm_feats
    ...
    def neg_log_likelihood(self, sentence, tags, device):
        feats = self._get_lstm_features(sentence, device)
        forward_score = self._forward_alg(feats)
        gold_score = self._score_sentence(feats, tags, device)
        return forward_score - gold_score

    def forward(self, sentence, device):  # dont confuse this with _forward_alg above.
        # Get the emission scores from the BiLSTM
        lstm_feats = self._get_lstm_features(sentence,device)
...
with torch.no_grad():
    ...
    print(precheck_sent.device)
    print(model(precheck_sent, device))

# Make sure prepare_sequence from earlier in the LSTM section is loaded
for epoch in tqdm.tqdm(range(300)):  # again, normally you would NOT do 300 epochs, it is toy data
    # for sentence, tags in training_data:
        ...
        for sentence, _ in training_data:
            print(prepare_sequence(sentence, word_to_ix).unsqueeze(0).size())

        sentence_in = tuple(prepare_sequence(sentence, word_to_ix).unsqueeze(0)  for sentence, _ in training_data)
        print(sentence_in)
